...
bool take_measurements::setup() {
  ...
  else Log.infoln("SHT31 initialized with temp %F and humidity %F", sht31.readTemperature(), sht31.readHumidity());
  ...
  if (!Presence::instance().setup()) {
    Log.infoln("Presence sensor failed to initialize");
    return false;
  }
...
bool take_measurements::setUpPresenceInterrupt(int sensitivity) {
  return true;

}

bool take_measurements::clearPresenceInterrupt() {

  TapSensor::instance().clearTapInts();
  return true;
